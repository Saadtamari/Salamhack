        ...
        {[
          ...
          <div key={s.label} style={{ background: "#FFFDF8", borderRadius: 12, padding: "20px 22px", border: "1px solid #DDD6CA" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              ...
              <div style={{ width: 40, height: 40, borderRadius: 12, background: "#F1EDE5", border: "1px solid #DDD6CA", color: "#11100E", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: "#FFFDF8", borderRadius: 20, padding: "24px", border: "1px solid #DDD6CA" }}>
          ...
          {overdue.length === 0
            ...
            : overdue.map((inv) => (
              <div key={inv.id} style={{ background: "#FFFDF8", borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <div>
                    ...
                    <button onClick={() => toast?.("تم إرسال رسالة المتابعة", "success")} style={{ fontSize: 11, color: "#11100E", background: "#F1EDE5", border: "none", borderRadius: 8, padding: "4px 10px", cursor: "pointer", marginTop: 4, fontFamily: "IBM Plex Sans Arabic" }}>ذكّر</button>
                  </div>
                </div>
              </div>
            ))
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "0 24px" }}>
          {DD.transactions.map((t) => (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid #F1EDE5" }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: t.type === "income" ? "#11100E" : "#11100E", flexShrink: 0 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <button onClick={() => setShowCreate(true)} style={{ background: "#11100E", color: "white", border: "none", borderRadius: 14, padding: "12px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "IBM Plex Sans Arabic" }}>
          + فاتورة جديدة
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[{ k: "all", l: "الكل" }, { k: "overdue", l: "متأخرة" }, { k: "sent", l: "مُرسلة" }, { k: "paid", l: "مدفوعة" }, { k: "draft", l: "مسودة" }].map((t) => (
          <button key={t.k} onClick={() => setFilter(t.k)} style={{ background: filter === t.k ? "#11100E" : "#FFFDF8", color: filter === t.k ? "white" : "#11100E", border: "1px solid #DDD6CA", borderRadius: 10, padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "IBM Plex Sans Arabic" }}>{t.l}</button>
        ))}
      </div>
      ...
      <div style={{ background: "#FFFDF8", borderRadius: 20, border: "1px solid #DDD6CA", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#F7F4EE", borderBottom: "1px solid #DDD6CA" }}>
          ...
          <div style={{ background: "#FFFDF8", borderRadius: 24, padding: "28px", width: 480, fontFamily: "IBM Plex Sans Arabic, sans-serif" }}>
            ...
            {["العميل", "وصف الخدمة", "المبلغ (USD)", "شروط الدفع"].map((f) => (
              <div key={f} style={{ marginBottom: 14 }}>
                ...
                {f === "العميل" ? (
                  <select style={{ width: "100%", padding: "11px 14px", border: "1px solid #DDD6CA", borderRadius: 12, fontSize: 14, fontFamily: "IBM Plex Sans Arabic", direction: "rtl", background: "#F7F4EE" }}>
                  ...
                  <input placeholder={f === "المبلغ (USD)" ? "0" : ""} style={{ width: "100%", padding: "11px 14px", border: "1px solid #DDD6CA", borderRadius: 12, fontSize: 14, fontFamily: "IBM Plex Sans Arabic", direction: "rtl", background: "#F7F4EE", boxSizing: "border-box" }} />
                )}
              </div>
            ))}
          ...
          <div key={c.id} onClick={() => setSelected(selected?.id === c.id ? null : c)}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                ...
                <div key={s.l} style={{ background: "#F7F4EE", borderRadius: 10, padding: "8px 10px" }}>
        ...
        <div style={{ background: "#FFFDF8", borderRadius: 20, padding: "24px", border: "1px solid #DDD6CA" }}>
          ...
          {DD.transactions.map((t) => (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: "1px solid #F1EDE5" }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 800, color: t.type === "income" ? "#11100E" : "#11100E" }}>
                {t.type === "income" ? "+" : ""}{fmt(Math.abs(t.amount))}
              </div>
            </div>
          ))}
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          {/* Hero card */}
          <div style={{ background: "#11100E", borderRadius: 20, padding: "28px", color: "white", marginBottom: 20 }}>
          ...
          <button onClick={() => toast?.("تم تسجيل دفع الزكاة. جزاك الله خيراً.", "success")} style={{ width: "100%", background: "#11100E", color: "white", border: "none", borderRadius: 16, padding: "16px", fontSize: 16, fontWeight: 700, cursor: "pointer", fontFamily: "IBM Plex Sans Arabic" }}>
            أدّ زكاتك الآن ✓
          </button>
        </div>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 420px", gap: 20 }}>
        <div>
          {/* Upload area */}
          <div
            style={{ background: "#F1EDE5", border: "2px dashed #11100E", borderRadius: 16, padding: "24px", textAlign: "center", marginBottom: 20, cursor: "pointer" }}
            onClick={() => toast?.("يرجى رفع ملف PDF", "info", "رفع عقد")}
          >
            <div style={{ fontSize: 36 }}>📎</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#11100E", marginTop: 8 }}>ارفع عقداً للتحليل</div>
            <div style={{ fontSize: 12, color: "#92897C", marginTop: 4 }}>يُحلّل الذكاء الاصطناعي البنود الخطرة فوراً</div>
          </div>

          {DD.contracts.map((c) => (
            <div key={c.id} onClick={() => setSelected(selected?.id === c.id ? null : c)}
              style={{ background: "#FFFDF8", borderRadius: 16, padding: "18px 20px", marginBottom: 12, border: `2px solid ${selected?.id === c.id ? "#11100E" : "#DDD6CA"}`, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                ...
                <div style={{ textAlign: "left" }}>
                  ...
                  {c.criticalFlags > 0
                    ? <span style={{ fontSize: 11, color: "#B3261E", background: "#FFFDF8", padding: "3px 8px", borderRadius: 8, fontWeight: 800, display: "inline-flex", alignItems: "center", gap: 5 }}><MasrafIcon icon={Alert02Icon} size={13} color="#B3261E" strokeWidth={2} />{c.criticalFlags} حرج</span>
                    : c.status === "pending"
                      ? <span style={{ fontSize: 11, color: "#9C7614", background: "#FFF9E8", padding: "3px 8px", borderRadius: 8 }}>⏳ جاري</span>
                      : <span style={{ fontSize: 11, color: "#11100E", background: "#FFFDF8", padding: "3px 8px", borderRadius: 8, display: "inline-flex", alignItems: "center", gap: 5 }}><MasrafIcon icon={CheckmarkCircle02Icon} size={13} color="#11100E" />آمن</span>
                  }
                </div>
              </div>
            </div>
            ...
            <div style={{ background: "#FFFDF8", borderRadius: 20, padding: "24px", border: "1px solid #DDD6CA" }}>
              ...
              {DD.contractFlags.map((f, i) => (
                <div key={i} style={{ background: "#FFFDF8", borderRadius: 14, padding: "16px", marginBottom: 12, border: "1px solid #DDD6CA" }}>
                  <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 10, background: f.severity === "critical" ? "#FFFDF8" : "#FFF9E8", color: f.severity === "critical" ? "#B3261E" : "#8A6400", fontWeight: 700 }}>
                  ...
                  <div style={{ background: "#F7F4EE", borderRadius: 10, padding: "10px 12px", fontSize: 12, color: "#11100E", display: "flex", alignItems: "center", gap: 8 }}>
                    <MasrafIcon icon={InformationCircleIcon} size={15} color="#11100E" />
                    {f.recommendation}
                  </div>
                </div>
              ))}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <button onClick={() => toast?.("جاري تنزيل تقرير أبريل PDF...", "success")} style={{ background: "#11100E", color: "#FFFDF8", border: "none", borderRadius: 12, padding: "12px 20px", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "var(--font-ar)", display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      {/* AI summary banner */}
      <div style={{ background: "#11100E", borderRadius: 20, padding: "24px", marginBottom: 24, color: "white", position: "relative", overflow: "hidden" }}>

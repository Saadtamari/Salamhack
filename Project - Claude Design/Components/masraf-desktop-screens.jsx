// ─── Masraf Desktop Screens ───────────────────────────────────────────────────
// All screens rewritten for wide desktop layout

const { useState: useDSt, useEffect: useDEf } = React;
const DD = window.MASRAF_DATA;

// ─── Desktop Dashboard ────────────────────────────────────────────────────────
function DesktopDashboard({ onNavigate, toast }) {
...
// ─── Desktop Invoices ─────────────────────────────────────────────────────────
function DesktopInvoices({ toast }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        ...
        <button onClick={() => setShowCreate(true)} style={{ background:"#1B5E20", color:"white", border:"none", borderRadius:14, padding:"12px 24px", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"IBM Plex Sans Arabic", display:"flex", alignItems:"center", gap:8 }}>
          + فاتورة جديدة
        </button>
      </div>
      ...
      <div style={{ background:"#FFFFFF", borderRadius:20, border:"1px solid #E8E5DE", overflow:"hidden" }}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>
            {filtered.map(inv => (
              <tr key={inv.id} onClick={() => setPreviewInv(inv)} style={{ borderBottom:"1px solid #F4EDE0", cursor:"pointer", transition:"background 0.12s" }} onMouseEnter={e => e.currentTarget.style.background="#FBF7F0"} onMouseLeave={e => e.currentTarget.style.background="transparent"}>
            ...
            {["العميل","وصف الخدمة","المبلغ (USD)","شروط الدفع"].map(f => (
              <div key={f} style={{ marginBottom:14 }}>
                ...
                {f==="العميل" ? (
                  ...
                  <input placeholder={f==="المبلغ (USD)"?"0":""} style={{ width:"100%", padding:"11px 14px", border:"1px solid #E8E5DE", borderRadius:12, fontSize:14, fontFamily:"IBM Plex Sans Arabic", direction:"rtl", background:"#FAFAF8" }} />
                )}
              </div>
            ))}
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns: selected ? "1fr 380px" : "repeat(3,1fr)", gap:16, transition:"all 0.3s" }}>
        {DD.clients.map(c => (
          <div key={c.id} onClick={() => setSelected(selected?.id===c.id ? null : c)} style={{ background:"#FFFFFF", borderRadius:20, padding:"20px", border:`2px solid ${selected?.id===c.id?"#1B5E20":"#E8E5DE"}`, cursor:"pointer", transition:"border 0.2s" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {[{l:"إجمالي الفواتير",v:fmt(c.totalInvoiced)},{l:"المتأخر",v:fmt(c.overdue),red:c.overdue>0}].map(s=>(
        ...
        {selected && (
          <div style={{ background:"#FFFFFF", borderRadius:20, padding:"24px", border:"1px solid #E8E5DE", gridRow:"1 / span 3" }}>
            ...
            {[["إجمالي الفواتير",fmt(selected.totalInvoiced)],["المدفوع",fmt(selected.totalPaid)],["المتأخر",fmt(selected.overdue)],["متوسط أيام الدفع",`${selected.avgDays} يوم`],["عدد الفواتير",`${selected.invoicesCount} فواتير`]].map(([l,v]) => (
...
// ─── Desktop Expenses ─────────────────────────────────────────────────────────
function DesktopExpenses({ toast }) {
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        <div>
          ...
          <div style={{ background:"#FFFFFF", borderRadius:20, padding:"24px", border:"1px solid #E8E5DE", marginBottom:20 }}>
            ...
            {[["إجمالي الأصول",fmt(z.totalAssets)],["خصم: المصاريف",`- ${fmt(DD.stats.totalExpenses)}`],["الوعاء الزكوي",fmt(z.eligible),true],["نسبة الزكاة","٢.٥٪"],["الزكاة المستحقة",fmt(z.amount),true,true]].map(([l,v,b,g])=>(
              <div key={l} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:"1px solid #F5F5F0", fontSize:13 }}>
                <span style={{ color:"#6B6B65" }}>{l}</span>
                <span style={{ fontWeight:b?800:500, color:g?"#C6930A":"#1A1A18" }}>{v}</span>
              </div>
            ))}
          </div>
...
// ─── Desktop Contracts ────────────────────────────────────────────────────────
function DesktopContracts({ toast }) {
  const [selected, setSelected] = useState(null);
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 420px", gap:20 }}>
        <div>
          <div style={{ background:"#F3E5F5", border:"2px dashed #7B1FA2", borderRadius:16, padding:"24px", textAlign:"center", marginBottom:20, cursor:"pointer" }}
          ...
          {DD.contracts.map(c => (
            <div key={c.id} onClick={() => setSelected(selected?.id===c.id?null:c)} style={{ background:"#FFFFFF", borderRadius:16, padding:"18px 20px", marginBottom:12, border:`2px solid ${selected?.id===c.id?"#7B1FA2":"#E8E5DE"}`, cursor:"pointer" }}>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <div>
                ...
                <div style={{ textAlign:"left" }}>
                  ...
                  {c.criticalFlags>0 ? <span style={{ fontSize:11, color:"#B71C1C", background:"#FFEBEE", padding:"3px 8px", borderRadius:8, fontWeight:700 }}>🚨 {c.criticalFlags} حرج</span>
                    : c.status==="pending" ? <span style={{ fontSize:11, color:"#C6930A", background:"#FFF8E1", padding:"3px 8px", borderRadius:8 }}>⏳ جاري</span>
      ...
      <div style={{ background:"linear-gradient(135deg,#1B5E20,#0D3B0F)", borderRadius:20, padding:"24px", marginBottom:24, color:"white", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          ...
          <div style={{ fontSize:15, lineHeight:1.8, maxWidth:700 }}>
            🤖 أداؤك المالي هذا الشهر <strong>ممتاز</strong>. دخلك ارتفع <strong>٦٧٪</strong> مقارنة بمارس وهو أعلى مستوى في ٦ أشهر. لديك <strong>عميلان</strong> يحتاجان متابعة تحصيل عاجلة. مصاريفك في حدود معقولة (<strong>٣١٪</strong> من الدخل). يُنصح بتأسيس صندوق طوارئ يعادل ٣ أشهر من المصاريف.
          </div>
        </div>
      </div>
